...
const StaffVaccination: React.FC = () => {
  ...
  const [childCode, setChildCode] = useState<string | undefined>(
    initialChildCode
  )
  ...
  const { data: children, isFetching: childrenFetching } =
  ...
  const handleStatusChangeClick = async (
    ...
    try {
      ...
      if (
        newStatus === 4 &&
        (!vaccination.vaccinator || !vaccination.vaccinator.id)
      ) {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'currentSequence',
      render: (currentSequence: number) => (
        <span style={{ fontSize: '12px' }}>Mũi thứ {currentSequence}</span>
      ),
    },
    {
      title: 'Lịch tiêm',
